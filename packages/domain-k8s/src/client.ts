...
/**
 * Loads the user's kubeconfig from the session so the observability board can query live cluster state on their behalf.
 */
...
/**
 * Produces a CoreV1 API client backed by the session's kubeconfig, letting the dashboard read pods, services, and configmaps.
 */
...
/**
 * Returns an AppsV1 client so the board can pull Deployments, StatefulSets, and DaemonSets directly from Kubernetes.
 */
...
/**
 * Provides a BatchV1 API client so scheduled Jobs and CronJobs shown on the board match the cluster state.
 */
...
/**
 * Creates a NetworkingV1 client which the board uses to inventory Services and Ingresses for exposure maps.
 */
