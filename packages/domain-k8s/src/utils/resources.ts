...
/** Cleans up raw Kubernetes quantity strings so downstream parsers feeding the observability board can rely on consistent input. */
...
/** Parses CPU strings (cores/millicores) into milli-core numbers so the board can graph requested capacity directly from Kubernetes specs. */
...
/** Helper that translates quantity suffixes (Gi, Mi, etc.) into numeric bytes for the dashboard. */
...
/** Parses Kubernetes memory quantity strings into raw bytes to power observability widgets. */
...
/** Formats milli-core totals back into human readable strings used in resource summaries on the board. */
...
/** Converts bytes into binary units so the board can present aggregated memory usage succinctly. */
...
/** Aggregates container-level resource strings into totals, enabling pod-level insights in the dashboard. */
const sumQuantities = (
...
/** Turns aggregated numeric totals into formatted strings the observability cards can show. */
...
/**
 * Sums CPU/memory requests and limits across all containers so the observability board can compare workload guarantees versus limits.
 */
