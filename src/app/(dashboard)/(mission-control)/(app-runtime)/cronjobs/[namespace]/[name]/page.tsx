...
import { getCronJobDetail } from '@domain-k8s/services/cronjob.service'
...
export default async function Page({ params }: PageProps) {
  ...
  let detail: Awaited<ReturnType<typeof getCronJobDetail>> | null = null
  let error: ReturnType<typeof parseK8sError> | null = null

  try {
    detail = await getCronJobDetail(namespace, name, {
      includeRaw: true,
      includeYaml: true,
    })
  } catch (err) {
  ...
  return (
    <ResourceDetailPage
      ...
      detail={detail}
      error={error}
      InfoCardComponent={CronJobInfoCard}
      breadcrumbs={[
        { label: 'Home', href: '/' },
        { label: 'CronJobs', href: '/cronjobs' },
        { label: namespace, href: `/cronjobs?namespace=${namespace}` },
      ]}
    >
      {(detail) => (
