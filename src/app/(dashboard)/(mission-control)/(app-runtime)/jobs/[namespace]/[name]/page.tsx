...
import { getJobDetail } from '@domain-k8s/services/job.service'
...
export default async function Page({ params }: PageProps) {
  ...
  try {
    detail = await getJobDetail(namespace, name, {
      includeRaw: true,
      includeYaml: true,
    })
  } catch (err) {
  ...
  return (
    <ResourceDetailPage
      ...
      detail={detail}
      error={error}
      InfoCardComponent={JobInfoCard}
      breadcrumbs={[
        { label: 'Home', href: '/' },
        { label: 'Jobs', href: '/jobs' },
        { label: namespace, href: `/jobs?namespace=${namespace}` },
      ]}
    >
      {(detail) => (
