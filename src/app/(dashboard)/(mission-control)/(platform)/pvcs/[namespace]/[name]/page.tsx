...
import { getPVCDetail } from '@domain-k8s/services/pvc.service'
...
export default async function Page({ params }: PageProps) {
  ...
  let detail: Awaited<ReturnType<typeof getPVCDetail>> | null = null
  let error: ReturnType<typeof parseK8sError> | null = null

  try {
    detail = await getPVCDetail(namespace, name, {
      includeRaw: true,
      includeYaml: true,
    })
  } catch (err) {
  ...
  return (
    <ResourceDetailPage
      ...
      detail={detail}
      error={error}
      InfoCardComponent={PVCInfoCard}
      breadcrumbs={[
        { label: 'Home', href: '/' },
        { label: 'PersistentVolumeClaims', href: '/pvcs' },
        { label: namespace, href: `/pvcs?namespace=${namespace}` },
      ]}
    >
      {(detail) => (
