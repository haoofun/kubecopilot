...
import {
  listCronJobs,
  getCronJobDetail,
} from '@/lib/k8s/services/cronjob.service'
...
export async function GET(
  ...
  try {
    if (segments.length === 0) {
      ...
      const result = await listCronJobs({ params })
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listCronJobs({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const result = await getCronJobDetail(namespace, name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError(
          'Missing cronjob identifier for event lookup',
          {
    ...
    throw new NotFoundError('CronJob resource path')
  } catch (error) {
    return createErrorResponse(error, 'cronjobs')
  }
}
