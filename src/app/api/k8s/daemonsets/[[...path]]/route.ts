...
import {
  listDaemonSets,
  getDaemonSetDetail,
} from '@/lib/k8s/services/daemonset.service'
...
export async function GET(
  ...
  try {
    if (segments.length === 0) {
      ...
      const result = await listDaemonSets({ params })
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listDaemonSets({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const result = await getDaemonSetDetail(namespace, name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError(
          'Missing daemonset identifier for event lookup',
          {
    ...
    throw new NotFoundError('DaemonSet resource path')
  } catch (error) {
    return createErrorResponse(error, 'daemonsets')
  }
}
