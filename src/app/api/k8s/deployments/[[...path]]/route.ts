...
import {
  listDeployments,
  getDeploymentDetail,
} from '@/lib/k8s/services/deployment.service'
...
const DETAIL_FLAGS = ['includeRaw', 'includeYaml', 'includePods'] as const
...
export async function GET(
  ...
  try {
    if (segments.length === 0) {
      ...
      const result = await listDeployments({ params })
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listDeployments({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const includePods = parseOptionalBoolean(searchParams.get('includePods'))

      const result = await getDeploymentDetail(namespace, name, {
        includeRaw,
        includeYaml,
        includePods: includePods ?? false,
      })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError(
          'Missing deployment identifier for event lookup',
          {
    ...
    throw new NotFoundError('Deployment resource path')
  } catch (error) {
    return createErrorResponse(error, 'deployments')
  }
}
