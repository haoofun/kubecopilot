...
import {
  listIngresses,
  getIngressDetail,
} from '@/lib/k8s/services/ingress.service'
...
export async function GET(
  ...
  try {
    if (segments.length === 0) {
      ...
      const result = await listIngresses({ params })
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listIngresses({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const result = await getIngressDetail(namespace, name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError(
          'Missing ingress identifier for event lookup',
          {
    ...
    throw new NotFoundError('Ingress resource path')
  } catch (error) {
    return createErrorResponse(error, 'ingresses')
  }
}
