...
import { listJobs, getJobDetail } from '@/lib/k8s/services/job.service'
...
export async function GET(
  ...
  try {
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listJobs({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const result = await getJobDetail(namespace, name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError('Missing job identifier for event lookup', {
    ...
    throw new NotFoundError('Job resource path')
  } catch (error) {
    return createErrorResponse(error, 'jobs')
  }
}
