...
import { listNodes, getNodeDetail } from '@/lib/k8s/services/node.service'
...
export async function GET(
  ...
  try {
    ...
    if (segments.length === 1) {
      ...
      const result = await getNodeDetail(name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 2 && segments[1] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError('Missing node identifier for event lookup', {
          uid: 'Provide uid',
        })
      }
    ...
    throw new NotFoundError('Node resource path')
  } catch (error) {
    return createErrorResponse(error, 'nodes')
  }
}
