...
import { listPVCs, getPVCDetail } from '@/lib/k8s/services/pvc.service'
...
export async function GET(
  ...
  try {
    if (segments.length === 0) {
      ...
      const result = await listPVCs({ params })
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listPVCs({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const result = await getPVCDetail(namespace, name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError('Missing PVC identifier for event lookup', {
    ...
    throw new NotFoundError('PVC resource path')
  } catch (error) {
    return createErrorResponse(error, 'pvcs')
  }
}
