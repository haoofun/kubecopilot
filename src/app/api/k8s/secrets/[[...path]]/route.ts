...
import { listSecrets, getSecretDetail } from '@/lib/k8s/services/secret.service'
...
export async function GET(
  ...
  try {
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listSecrets({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const result = await getSecretDetail(namespace, name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError(
          'Missing secret identifier for event lookup',
          {
    ...
    throw new NotFoundError('Secret resource path')
  } catch (error) {
    return createErrorResponse(error, 'secrets')
  }
}
