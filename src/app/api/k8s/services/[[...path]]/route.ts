...
import {
  listServices,
  getServiceDetail,
} from '@/lib/k8s/services/service.service'
...
const DETAIL_FLAGS = ['includeRaw', 'includeYaml', 'includeEndpoints'] as const
...
export async function GET(
  ...
  try {
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listServices({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const includeEndpoints = parseOptionalBoolean(
        searchParams.get('includeEndpoints'),
      )

      const result = await getServiceDetail(namespace, name, {
        includeRaw,
        includeYaml,
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError(
          'Missing service identifier for event lookup',
          {
    ...
    throw new NotFoundError('Service resource path')
  } catch (error) {
    return createErrorResponse(error, 'services')
  }
}
