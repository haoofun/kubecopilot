...
} from '@/lib/k8s/services/statefulset.service'
...
export async function GET(
  ...
  try {
    if (segments.length === 0) {
      ...
      const result = await listStatefulSets({ params })
    ...
    if (segments.length === 1) {
      ...
      const params = buildQueryParams(searchParams)
      const result = await listStatefulSets({ namespace, params })
      return createDataResponse(result)
    }
    ...
    if (segments.length === 2) {
      ...
      const result = await getStatefulSetDetail(namespace, name, {
        includeRaw,
        includeYaml,
      })
      return createDataResponse(result)
    ...
    if (segments.length === 3 && segments[2] === 'events') {
      ...
      if (!uid && !nameParam) {
        throw new ValidationError(
          'Missing statefulset identifier for event lookup',
          {
    ...
    throw new NotFoundError('StatefulSet resource path')
  } catch (error) {
    return createErrorResponse(error, 'statefulsets')
  }
}
